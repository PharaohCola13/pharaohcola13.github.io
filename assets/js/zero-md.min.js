class t extends HTMLElement {
    ...
    constructor(t) {
        ...
        this.config = {
            ...
            cssUrls: ["assets/css/about.css"],
            ...t,
            ...window.ZeroMdConfig
        },
        this.root = this.hasAttribute("no-shadow") ? this : this.attachShadow({
            mode: "open"
        }),
    ...
    disconnectedCallback() {
        this.connected = !1,
        this.shadowRoot && this.shadowRoot.removeEventListener("click", this.clicked)
    }
    waitForReady() {
        const t = this.connected || new Promise((t=>{
            this.addEventListener("zero-md-connected", (function e() {
                this.removeEventListener("zero-md-connected", e),
                t()
            }
            ))
        }
    ...
    onload(t) {
        return new Promise(((e,s)=>{
            t.onload = e,
            t.onerror = t=>s(t.path ? t.path[0] : t.composedPath()[0])
        }
        ))
    }
    loadScript(t) {
        return t = this.arrify(t),
        Promise.all(t.map((t=>{
            const [e,...s] = this.arrify(t)
              , r = document.createElement("script");
            return r.src = e,
            r.async = !1,
            s.forEach((t=>r.setAttribute(t, ""))),
            this.onload(document.head.appendChild(r))
        }
        )))
    }
    goto(t) {
    ...
    clicked(t) {
        if (t.metaKey || t.ctrlKey || t.altKey || t.shiftKey || t.defaultPrevented)
            return;
        const e = t.target.closest("a");
        e && e.hash && e.host === location.host && e.pathname === location.pathname && this.goto(e.hash)
    }
    clearDom() {
        this.root.querySelectorAll("[class^=markdown]").forEach((t=>t.remove()))
    }
    makeFrag(t) {
    ...
    getBaseUrl(t) {
        ...
        return e.href = t,
        e.href.substring(0, e.href.lastIndexOf("/") + 1)
    }
    highlight(t) {
        ...
        window.Prism.highlightAllUnder(t)
    }
    buildStyles() {
        const t = t=>{
          ...
          , s = `<div class="markdown-styles"><style>${this.config.hostCss}</style>${t('template[data-merge="prepend"]')}${t("template:not([data-merge])") || e.reduce(((t,e)=>`${t}<link rel="stylesheet" href="${e}">`), "")}${t('template[data-merge="append"]')}</div>`;
        return this.makeFrag(s)
    }
    async buildMd(t={}) {
        const e = `<div class="markdown-body${t.classes ? this.arrify(t.classes).reduce(((t,e)=>`${t} ${e}`), " ") : ""}">${await (async()=>{
            if (!this.src)
                return "";
            const e = await fetch(this.src);
            if (e.ok) {
                const s = await e.text();
                return window.marked(s, {
                    baseUrl: this.getBaseUrl(this.src),
                    ...t
                })
            }
            return this.fire("zero-md-error", {
                msg: `[zero-md] HTTP error ${e.status} while fetching src`,
                status: e.status,
                src: this.src
            }),
            ""
            ...
            return window.marked(s, t)
        }
        )()}</div>`
          , s = this.makeFrag(e);
        return this.highlight(s.firstElementChild),
        s
    }
    async stampDom(t) {
        const e = [...t.querySelectorAll('link[rel="stylesheet"]')];
        this.root.appendChild(t.firstElementChild),
        await Promise.all(e.map((t=>this.onload(t)))).catch((t=>{
    ...
    async render(t={}) {
        ...
        this.clearDom();
        const e = this.buildStyles()
          , s = this.buildMd(t);
        await this.stampDom(e),
        await this.tick(),
        await this.stampDom(await s),
