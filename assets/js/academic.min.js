    ...
    (()=>{
        var e = {
          ...
          , d = {
            ...
            distance: 100,
            maxPatternLength: 32,
            minMatchCharLength: c.minLength,
            keys: [{
                name: "title",
                weight: .99
            }, {
        ...
        function b(b, c) {
            ...
            !b && a.length < d.minMatchCharLength)
                return;
            $("#search-hits").empty(),
            $("#search-common-queries").hide(),
            i(a, c);
            let e = window.location.protocol + "//" + window.location.host + window.location.pathname + "?q=" + encodeURIComponent(a) + window.location.hash;
            h(e)
        }
        function i(b, c) {
            let a = c.search(b);
            a.length > 0 ? ($("#search-hits").append('<h3 class="mt-0">' + a.length + " " + f.results + "</h3>"),
            j(b, a)) : $("#search-hits").append('<div class="search-no-results">' + f.no_results + "</div>")
        }
        function j(b, c) {
            $.each(c, function(j, c) {
                let f = c.item.section
                  , g = ""
                  , h = ""
                  , i = [];
                ["publication", "event"].includes(f) ? g = c.item.summary : g = c.item.content,
                d.tokenize ? i.push(b) : $.each(c.matches, function(c, b) {
                    if (b.key == "content") {
                        let c = b.indices[0][0] - a > 0 ? b.indices[0][0] - a : 0
                          , d = b.indices[0][1] + a < g.length ? b.indices[0][1] + a : g.length;
                        h += g.substring(c, d),
                        i.push(b.value.substring(b.indices[0][0], b.indices[0][1] - b.indices[0][0] + 1))
                    }
                }),
                h.length < 1 && (h += c.item.summary);
                ...
                let m = {
                    ...
                    title: c.item.title,
                    type: f,
                    relpermalink: c.item.relpermalink,
                    snippet: h
                }
                  , n = k(l, m);
                $("#search-hits").append(n),
                $.each(i, function(b, a) {
                    $("#summary-" + j).mark(a)
                })
            })
        ...
        typeof Fuse == "function" && $.getJSON(c.indexURI, function(e) {
            let a = new Fuse(e,d)
              , c = g("q");
            c && ($("body").addClass("searching"),
            $(".search-results").css({
                opacity: 0,
                visibility: "visible"
            }).animate({
            ...
            $("#search-query").focus(),
            b(!0, a)),
            $("#search-query").keyup(function(c) {
                clearTimeout($.data(this, "searchTimer")),
                c.keyCode == 13 ? b(!0, a) : $(this).data("searchTimer", setTimeout(function() {
                    b(!1, a)
                }, 250))
            })
        })
